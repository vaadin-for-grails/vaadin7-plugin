...
class DomainItem<T> implements Item {
    ...
    final T object
    final Map<String, org.vaadin.grails.data.util.DomainItemProperty<T, ?>> propertyById = new HashMap()
    ...
    DomainItem(T object) {
        ...
        domainClass.getPersistentProperties().each { property ->
            ...
            addItemProperty(id, new org.vaadin.grails.data.util.DomainItemProperty<T, Object>(this, id))
        }
    }
    ...
    @Override
    org.vaadin.grails.data.util.DomainItemProperty<T, ?> getItemProperty(Object id) {
        propertyById.get(id)
    }
